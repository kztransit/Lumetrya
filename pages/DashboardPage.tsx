...
const EmptyState: React.FC = () => (
    <div className="text-center py-10 bg-white dark:bg-slate-800 rounded-xl shadow-md">
        <h2 className="text-xl font-semibold text-slate-800 dark:text-slate-100">Нет данных для отображения</h2>
        ...
        <div className="space-y-6">
            ...
            <div className="flex flex-col md:flex-row justify-between md:items-center gap-4">
                <div>
                    ...
                    <p className="text-slate-500 dark:text-slate-400 mt-1">Комплексная сводка по всем маркетинговым кампаниям и продажам</p>
                </div>
                <button
                ...
                <div className="bg-white dark:bg-slate-800 p-6 rounded-xl shadow-md h-full flex flex-col">
                    ...
                    <div className="flex flex-col items-center justify-center flex-1 space-y-2">
                        {dashboardData.funnelData.map((item, index) => {
                            ...
                            // Ensure minimum width for text visibility, e.g., at least 40%
                ...
                <div className="bg-white dark:bg-slate-800 p-6 rounded-xl shadow-md">
                    ...
                    <div className="grid grid-cols-2 gap-4 h-full">
                        ...
                        <div className="space-y-3">
                            <div><p className="text-sm text-slate-500 dark:text-slate-400">CPC</p><p className="font-bold text-lg dark:text-slate-200">{formatCurrency(dashboardData.cpc)}</p></div>
                            <div><p className="text-sm text-slate-500 dark:text-slate-400">CPL</p><p className="font-bold text-lg dark:text-slate-200">{formatCurrency(dashboardData.cpl)}</p></div>
                            <div><p className="text-sm text-slate-500 dark:text-slate-400">ROI</p><p className="font-bold text-lg text-purple-600 dark:text-purple-400">{dashboardData.roi.toFixed(1)}%</p></div>
                            <div><p className="text-sm text-slate-500 dark:text-slate-400">Ср. чек</p><p className="font-bold text-lg dark:text-slate-200">{formatCurrency(dashboardData.averageCheck)}</p></div>
                        </div>
                    </div>
                </div>
                        ...
                        {filteredReports.slice(-12).reverse().map(r => {
                            ...
                            return (
                                <tr key={r.id} className="border-b border-gray-200 dark:border-slate-700">
                                    <td className="px-4 py-3 font-medium text-slate-900 dark:text-slate-100 whitespace-nowrap">{r.name.replace('Отчет ', '')}</td>
                                    ...
                                    <td className="px-4 py-3 whitespace-nowrap font-semibold text-blue-600 dark:text-blue-400">{formatCurrency(avgCheck)}</td>
                                    <td className="px-4 py-3 whitespace-nowrap">{formatCurrency(cpl)}</td>
                                    <td className={`px-4 py-3 font-semibold whitespace-nowrap ${roi >= 0 ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400'}`}>{roi.toFixed(1)}%</td>
                                </tr>
                            )
                        })}
                    </tbody>
